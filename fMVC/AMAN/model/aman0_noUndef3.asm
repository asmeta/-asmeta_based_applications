...
//	LTLSPEC (forall $a in Airplane, $t in TimeSlot with g(position($a) = $t and isUndef(selectedAirplane) and action = HOLD implies x(position($a) = $t)))
	...
	LTLSPEC (forall $a in Airplane, $t in TimeSlot with g(position($a) = $t and selectedAirplane=$a and action = UP and canBeMovedUp($a) 
		and not blocked($t +1) implies x(position($a) = $t + 1)
	))
