...
definitions:		
	...
	macro rule r_check_inc_insp($insp in Integer,$exp in Integer) =
		if not((pi_6 > peep_max and count_ppmax >= 3) or ((div($insp,$exp))<0.01 and count_ie >= 4)) then
			par
				insp_valve := iValve
				exp_valve := oValve
			endpar
	...
	macro rule r_check_inc_exp =
		if not(pi_6 > peak_max and count_pkmax > 2) then
			par
				insp_valve := iValve
				exp_valve := oValve
			endpar
